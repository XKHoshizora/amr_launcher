...
options = {
    ...
    num_subdivisions_per_laser_scan = 1,  -- 每次激光雷达扫描的细分数
    num_point_clouds = 0,  -- 点云数量
    lookup_transform_timeout_sec = 0.3,  -- 查找坐标变换的超时时间（秒）
    submap_publish_period_sec = 0.3,  -- 子图发布周期（秒）
    pose_publish_period_sec = 0.05,  -- 位姿发布周期（秒）
    trajectory_publish_period_sec = 0.05,  -- 轨迹发布周期（秒）
...
MAP_BUILDER.num_odometry_states = 1  -- 里程计状态数量

-- 激光雷达配置
TRAJECTORY_BUILDER_2D.min_range = 0.1  -- 最小有效测距范围
TRAJECTORY_BUILDER_2D.max_range = 25.0  -- 最大有效测距范围
...
TRAJECTORY_BUILDER_2D.use_online_correlative_scan_matching = true  -- 是否使用在线扫描匹配
TRAJECTORY_BUILDER_2D.voxel_filter_size = 0.05  -- 体素滤波器尺寸

-- 扫描匹配配置
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 10  -- 平移权重
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 30  -- 旋转权重

-- 子图配置
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 40  -- 每个子图的激光数据数量
TRAJECTORY_BUILDER_2D.submaps.grid_options_2d.resolution = 0.05  -- 子图的分辨率

-- 回环检测配置
POSE_GRAPH.optimize_every_n_nodes = 40  -- 每隔多少节点进行一次优化
